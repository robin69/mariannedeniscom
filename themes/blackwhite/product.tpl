...
<div id="primary_block">
	...
	<div id="pb-right-column">
		...
		<div id="usefull_link_block">
		  ...
		  <span id="usefull_link_block_send">{if $HOOK_EXTRA_LEFT}{$HOOK_EXTRA_LEFT}{/if}</span> 
	...
	<h2>{$product->name|escape:'htmlall':'UTF-8'}</h2>
		...
		<div id="color_picker">
			...
			{foreach from=$colors key='id_attribute' item='color'}
				<a id="color_{$id_attribute|intval}" class="color_pick" onclick="updateColorSelect({$id_attribute|intval});">{if file_exists($col_img_dir|cat:$id_attribute|cat:'.jpg')}<img src="{$img_col_dir}{$id_attribute}.jpg" alt="" title="{$color.name}" />{/if}</a>
			{/foreach}
		</div><br /><br /><br />
        <div class="clear"></div>
		{/if}
		...
		<form id="buy_block" action="{$base_dir}cart.php" method="post">
			...
			<input type="submit" name="Submit" value="{l s='Add to cart'}" /></sp><br /><br />
			...
			{assign var='accessoryLink' value=$link->getProductLink($accessory.id_product, $accessory.link_rewrite, $accessory.category)}
			  <li class="ajax_block_product {if $smarty.foreach.accessories_list.first}first_item{elseif $smarty.foreach.accessories_list.last}last_item{else}item{/if} product_accessories_description">
				<p>
				 ...
				 <img src="{$img_prod_dir}{$accessory.id_image}-medium.jpg" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}" /></a>
				</p>
				<h5>
