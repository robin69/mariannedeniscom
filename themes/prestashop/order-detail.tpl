	...
	<table class="std">
		<thead>
			<tr>
				...
				<th class="{if $return_allowed}item{else}first_item{/if}" style="padding-left:0;">{l s='Reference'}</th>
			...
			{if !$product.deleted}
				...
				{if isset($customizedDatas.$productId.$productAttributeId)}
					<tr class="item">
						...
						<td style="padding-left:0;"><label for="cb_{$product.id_order_detail|intval}">{if $product.product_reference}{$product.product_reference|escape:'htmlall':'UTF-8'}{else}--{/if}</label></td>
						<td class="bold">
					...
					<tr class="alternate_item">
						...
						{foreach from=$customization.datas key='type' item='datas'}
							...
							<ul class="typedText">{counter start=0 print=false}
								{foreach from=$datas item='data'}
									<li>{$data.name}{l s=':'} : <strong>{$data.value}</strong></li>
								{/foreach}
							</ul>
							{/if}
						{/foreach}
