...
<h2 class="page_title_h2">{l s='Shopping cart summary'}</h2>
...
<div class="my-account-container">
	...
	<div id="order-detail-content" class="table_block">
		<table id="cart_summary" class="std">
			...
			<thead>
				<tr>
					<th class="cart_product first_item" style="padding-left:10px;">{l s='Product'}</th>
					<th class="cart_description item">{l s='Description'}</th>
					<th class="cart_ref item">{l s='Ref.'}</th>
					<th class="cart_unit item">{l s='Unit price'}</th>
					<th class="cart_quantity item">&nbsp;{l s='Qty'}</th>
					<th class="cart_total last_item" style="text-align:right;padding-right:20px;">{l s='Total'}</th>
				</tr>
			</thead>
			...
			<tfoot>
				...
				<tr class="cart_total_price">
					...
					<td class="price" style="padding-right:10px;">{displayPrice price=$total_price_without_tax}</td>
				</tr>
				<tr class="cart_total_voucher">
					<td colspan="5">{l s='Total tax:'}</td>
					<td class="price" style="padding-right:10px;">{displayPrice price=$total_tax}</td>
				</tr>
				
					{if $priceDisplay}
						<tr class="cart_total_price">
							<td colspan="5">{l s='Total products (tax excl.):'}</td>
							<td class="price" style="padding-right:10px;">{displayPrice price=$total_products}</td>
						</tr>
					{else}
						<tr class="cart_total_price">
							<td colspan="5">{l s='Total products (tax incl.):'}</td>
							<td class="price" style="padding-right:10px;">{displayPrice price=$total_products_wt}</td>
						</tr>
					{/if}
					...
					{if $shippingCost > 0}
						{if $priceDisplay}
							<tr class="cart_total_delivery">
								...
								<td class="price" style="padding-right:10px;">{displayPrice price=$shippingCostTaxExc}</td>
							</tr>
						{else}
							<tr class="cart_total_delivery">
								<td colspan="5">{l s='Total shipping (tax incl.):'}</td>
								<td class="price" style="padding-right:10px;">{displayPrice price=$shippingCost}</td>
							</tr>
						{/if}
					{/if}
				...
				<tr class="cart_total_price">
					...
					<td class="price" style="padding-right:10px;">{displayPrice price=$total_price}</td>
				</tr>
				{else}
				<tr class="cart_total_price">
					<td colspan="5" class="big-cart-text">TOTAL</td>
					<td class="big-cart-text" style="padding-right:10px;" >{displayPrice price=$total_price_without_tax}</td>
				</tr>
				
					<tr class="cart_total_price">
						<td colspan="5" class="small-cart-text">dont total produits</td>
						<td class="price small-cart-text" style="padding-right:10px;">{displayPrice price=$total_products}</td>
					</tr>
					
					{if $shippingCost > 0}
						<tr class="cart_total_delivery">
							<td colspan="5" class="small-cart-text">dont total frais de port</td>
							<td class="price small-cart-text" style="padding-right:10px;">{displayPrice price=$shippingCostTaxExc}</td>
						</tr>
					{/if}
				...
				{if $total_discounts != 0}
					{if $use_taxes}
						{if $priceDisplay}
							<tr class="cart_total_voucher">
								<td colspan="5">{l s='Total vouchers (tax excl.):'}</td>
								<td class="price-discount" style="padding-right:10px;">{displayPrice price=$total_discounts_tax_exc}</td>
							</tr>
						{else}
							<tr class="cart_total_voucher">
								<td colspan="5">{l s='Total vouchers (tax incl.):'}</td>
								<td class="price-discount" style="padding-right:10px;">{displayPrice price=$total_discounts}</td>
							</tr>
						{/if}
					{else}
						<tr class="cart_total_voucher">
				...
				{if $total_wrapping > 0}
					{if $use_taxes}
						{if $priceDisplay}
							<tr class="cart_total_voucher">
								<td colspan="5">{l s='Total gift-wrapping (tax excl.):'}</td>
								<td class="price-discount" style="padding-right:10px;">{displayPrice price=$total_wrapping_tax_exc}</td>
							</tr>
						{else}
							<tr class="cart_total_voucher">
								<td colspan="5">{l s='Total gift-wrapping (tax incl.):'}</td>
								<td class="price-discount" style="padding-right:10px;">{displayPrice price=$total_wrapping}</td>
							</tr>
						{/if}
					{else}
						<tr class="cart_total_voucher">
...
<tr class="cart_free_shipping">
					...
					<td class="price" style="padding-right:10px;">{displayPrice price=$free_ship}</td>
				</tr>
-->
				{/if}
			</tfoot>
					...
					{foreach from=$customizedDatas.$productId.$productAttributeId key='id_customization' item='customization'}
						...
						<tr class="alternate_item cart_item">
							<td colspan="4">
								{foreach from=$customization.datas key='type' item='datas'}
									...
									{elseif $type == $CUSTOMIZE_TEXTFIELD}
										<ul class="typedText">
											{foreach from=$datas item='textField' name='typedText'}<li>{if $textField.name}{$textField.name}{else}{l s='Text #'}{$smarty.foreach.typedText.index+1}{/if}{l s=':'} : <strong>{$textField.value}</strong></li>{/foreach}
										</ul>
									{/if}
								{/foreach}
							</td>
							<td class="cart_quantity">
								...
								<p class="cart_quantity_nb">{$customization.quantity}</p>
		...
		<form action="{$base_dir_ssl}order.php" method="post" id="voucher">
			<fieldset>
				...
				<p>
					...
					<input type="text" id="discount_name" name="discount_name" value="{if $discount_name}{$discount_name}{/if}" /> <input type="submit" name="submitAddDiscount" value="" class="button button_add_voucher submit" /><input type="hidden" name="submitDiscount" />
				</p>
				<!-- <p class="submit"></p> -->
			</fieldset>
		</form>
	...
	<p class="cart_navigation">
		<a href="{$base_dir_ssl}order.php?step=1{if $back}&amp;back={$back}{/if}" class="exclusive" title="">{l s='Next'} &raquo;</a>
		<a href="{if $smarty.server.HTTP_REFERER && strstr($smarty.server.HTTP_REFERER, 'order.php')}{$base_dir}index.php{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="button_large" title="">&laquo; {l s='Continue shopping'}</a>
	</p>
