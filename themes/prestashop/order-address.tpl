...
<h2 class="page_title_h2">{l s='Addresses'}</h2>
	...
	<form action="{$base_dir_ssl}order.php" method="post">
		<div class="addresses">
			...
			{if $addresses|@count > 1}
				...
				{else}
					<br /><a href="{$base_dir_ssl}address.php?back=order.php&amp;step=1&select_address=1{if $back}&mod={$back}{/if}" title="{l s='Add'}" class="button_large">{l s='Add a new address'}</a>
				{/if}
			</p>
			<div class="clear"></div>
			<ul class="address item" id="address_delivery">
				<li class="address_title">
					...
					<div class="address_update address_update_cart"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;back=order.php&amp;step=1{if $back}&mod={$back}{/if}" title="{l s='Update'}"></a></div>
				</li>
				...
				<li class="address_update"><a style="display:none;" href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;back=order.php&amp;step=1{if $back}&mod={$back}{/if}" title="{l s='Update'}">{l s='Update'}</a></li>
			</ul>
			...
			<div id="ordermsg">
				...
				<p class="textarea"><textarea name="message">{$oldMessage}</textarea></p>
			</div>
		</div>
