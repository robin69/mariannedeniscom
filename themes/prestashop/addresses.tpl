...
<h2 class="page_title_h2">{l s='My addresses'}</h2>

{if $logged}
	{include file=$tpl_dir./account-icons.tpl}
{/if}
...
<div class="my-account-container">
	...
	<div class="addresses">
		...
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}" style="margin-right:20px;">
			<li class="address_title">
				{$address.alias}
				<div class="address_update"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}" title="{l s='Update'}"></a></div>
				<div class="address_delete"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;delete" onclick="return confirm('{l s='Are you sure?'}');" title="{l s='Delete'}"></a></div>
			</li>
