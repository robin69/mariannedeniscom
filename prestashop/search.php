...
elseif ($tag = Tools::getValue('tag') AND !is_array($tag))
{
	...
	include(dirname(__FILE__).'/pagination.php');
	$smarty->assign(array('tag' => $tag, 'products' => Search::searchTag(intval($cookie->id_lang), $tag, false, $p, $n, $orderBy, $orderWay), 'nbProducts' => $nbProducts, 'homeSize' => Image::getSize('home')));
}
else
{
	$smarty->assign(array(
