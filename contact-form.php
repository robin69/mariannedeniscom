...
if (Tools::isSubmit('submitMessage'))
{
    ...
    elseif (!($id_contact = intval(Tools::getValue('id_contact'))) OR !(Validate::isLoadedObject($contact = new Contact(intval($id_contact), intval($cookie->id_lang)))))
		...
		elseif (!($prenom = nl2br2(Tools::getValue('prenom'))))  
        $errors[] = Tools::displayError('Veuillez renseigner votre prenom');
		elseif (!($nom = nl2br2(Tools::getValue('nom'))))  
        $errors[] = Tools::displayError('Veuillez renseigner votre nom');
